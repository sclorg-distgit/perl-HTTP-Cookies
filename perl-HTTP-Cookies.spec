%{?scl:%scl_package perl-HTTP-Cookies}

Name:           %{?scl_prefix}perl-HTTP-Cookies
Version:        6.08
Release:        3%{?dist}
...
BuildRequires:  %{?scl_prefix}perl(HTTP::Headers::Util) >= 6
BuildRequires:  %{?scl_prefix}perl(HTTP::Request)
BuildRequires:  %{?scl_prefix}perl(locale)
# Win32 not supported
# Tests:
BuildRequires:  %{?scl_prefix}perl(File::Spec)
BuildRequires:  %{?scl_prefix}perl(HTTP::Response)
...
Requires:       %{?scl_prefix}perl(HTTP::Headers::Util) >= 6
...
%changelog
* Fri Feb 14 2020 Petr Pisar <ppisar@redhat.com> - 6.08-3
- Import to SCL
